...
export const getFilteredProducts = filter => {
  ...
  return products.filter(dish => {
    return !(filter.categories && !!filter.categories.length && !filter.categories.includes(dish.category_id) || filter.restaurants && !!filter.restaurants.length && !filter.restaurants.includes(dish.restaurant_id) || filter.minPrice && !(calculatedPrice(dish) >= filter.minPrice) || filter.maxPrice && !(calculatedPrice(dish) <= filter.maxPrice) || filter.search && !dish.name.toLowerCase().includes(filter.search.toLowerCase()) || filter.rating && !(dish.review.stars >= filter.rating));
  });
};
