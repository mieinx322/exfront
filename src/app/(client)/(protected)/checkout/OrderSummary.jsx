...
const OrderSummary = () => {
  ...
  return <div className="mb-5">
      <div className="rounded-lg border border-default-200 p-5">
        ...
        return dish && <div key={item.id + dish?.id} className="mb-4 flex items-center">
                <img src={dish.images[0]} height={80} width={80} className="me-2 h-20 w-20" alt={dish.name} />
                <div>
        ...
        <button type="submit" className="inline-flex w-full items-center justify-center rounded-lg border border-primary bg-primary px-10 py-3 text-center text-sm font-medium text-white shadow-sm transition-all duration-500 hover:bg-primary-500">
          Place Order
        </button>
      </div>
    </div>;
