...
const ProductDetail = () => {
  ...
  useEffect(() => {
    (async () => {
      const foundDish = await getDishById(Number(dishId));
      if (!foundDish) {
        navigate('/not-found');
      } else {
        setDish(foundDish);
      ...
      <section className="py-6 lg:py-10">
        <div className="container">
          ...
          <div className="mb-10 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
            {dishesData.slice(0, 4).map((dish, idx) => <ProductGridCard key={idx} dish={dish} />)}
          </div>
          ...
          <div className="pt-10">
            ...
            {consumerReviews.map((review, idx) => <div key={review.id} className={cn("py-5", consumerReviews.length !== idx + 1 && "border-b border-default-200")}>
              <div className="mb-3 flex items-center">
                <img src={review.reviewer_image} alt="avatar" height={48} width={48} className="me-4 h-12 w-12 rounded-full" />
                <div>
                  ...
                  <div className="flex gap-1.5">
                    {Array.from(new Array(Math.floor(review.stars))).map((_val, idx) => <FaStar key={idx} className="text-base text-yellow-400" />)}
                    {!Number.isInteger(review.stars) && <FaStarHalfStroke size={18} className="text-yellow-400" />}
                    {review.stars < 5 && Array.from(new Array(5 - Math.ceil(review.stars))).map((_val, idx) => <FaStar key={idx} size={16} className="text-default-200" />)}
                  </div>
                </div>
              </div>
