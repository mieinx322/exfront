          ...
          <div className="grid gap-10 lg:grid-cols-6">
            <div className="lg:col-span-4">
              <div className="hs-accordion-group space-y-4">
                ...
                return <div key={idx} className={cn('hs-accordion overflow-hidden rounded-lg border border-default-200 hs-accordion-active:border-primary hs-accordion-active:shadow-[0px_8px_20px_0px_rgba(0,0,0,0.1)]', {
                  active: idx == 0
                })} id={`faq-${idx}`}>
                      <button className="hs-accordion-toggle inline-flex w-full items-center justify-between gap-x-3 px-6 py-4 text-left capitalize text-default-950 transition-all hs-accordion-active:bg-primary hs-accordion-active:text-white" aria-controls={`faq-accordion-${idx}`}>
                      ...
                      <div id={`faq-accordion-${idx}`} className={cn('hs-accordion-content w-full overflow-hidden transition-[height] duration-300', {
                    hidden: idx != 0
                  })} aria-labelledby={`faq-${idx}`}>
                        <div className="px-6 py-4">
                          <p className="mb-4 text-sm font-medium text-default-600">{faq.description}</p>
                          {faq.points && <ul className="list-disc ps-6 text-default-600">
                              {faq.points.map((point, idx) => {
                          return <li key={idx} className="mb-1">
                                    {point}
                                  </li>;
                        })}
                            </ul>}
