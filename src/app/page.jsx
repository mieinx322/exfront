...
import { adminPanelDemosData, authDemosData, clientAppDemosData, extraDemosData, frameworksData, landingFeaturesData, mainDemosData } from '@/assets/data/landing';
...
export default function Home() {
  return <>
          ...
          <div className="container">
            <nav className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <Link to="/">
                  <img src={logoDarkImg} width={130} height={40} alt="logo" className="flex h-10 dark:hidden" />
                  <img src={logoLightImg} width={130} height={40} alt="logo" className="hidden h-10 dark:flex" />
                </Link>
              </div>
              ...
              <div>
                <a href="#demo" className="inline-flex items-center gap-2.5 rounded bg-primary px-4 py-2 text-center text-sm font-semibold text-white shadow-lg  shadow-transparent transition-all duration-500 ease-in-out hover:-translate-y-[2px] focus:outline-none md:me-0">
        ...
        <div className="container relative">
          <div className="grid gap-12 lg:grid-cols-10">
            <div className="z-10 self-center lg:col-span-4">
              <div className="text-center lg:text-start">
                ...
                <a href="#demo" className="mt-10 inline-flex w-auto items-center gap-2 rounded-full bg-primary px-6 py-2.5 font-semibold text-white transition-all duration-300 hover:shadow-xl hover:shadow-primary-200/10">
                  View Demos <LuEye size={20} />
                </a>
              </div>
            </div>
            <div className="relative flex items-center justify-center lg:col-start-6 lg:col-end-11">
              <img alt="demo" src={demo1Img} width={728} height={527} className="mx-auto rounded-lg shadow-lg" />
            </div>
          </div>
          <div className="mt-20 text-center">
            ...
            <div className="mt-8 inline-flex flex-wrap items-center justify-center gap-4">
              {frameworksData.map((framework, idx) => <div key={framework.name + idx} className="flex h-16 w-16 items-center justify-center rounded-full bg-white dark:bg-default-200">
                  <img src={framework.icon} height={36} width={36} alt={framework.name} className="h-9 w-9" />
                </div>)}
            </div>
          </div>
        </div>
      ...
      <section className="py-20" id="demo">
        <div className="container">
          ...
          <div className="justify-content-center grid grid-cols-1 gap-8 lg:grid-cols-2">
            {mainDemosData.map((demo, idx) => <DemoCard key={idx} demo={demo} />)}
          </div>
        </div>
      </section>
      <section className="py-20">
        <div className="container">
          ...
          <div className="justify-content-center grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {clientAppDemosData.map((demo, idx) => <DemoCard key={idx} demo={demo} />)}
          </div>
        </div>
      </section>
      <section className="py-20" id="admin">
        <div className="container">
          ...
          <div className="justify-content-center grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {adminPanelDemosData.map((demo, idx) => <DemoCard key={idx} demo={demo} />)}
          </div>
        </div>
      </section>
      <section className="py-20">
        <div className="container">
          ...
          <div className="justify-content-center grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {authDemosData.map((demo, idx) => <DemoCard key={idx} demo={demo} />)}
          </div>
        </div>
      </section>
      <section className="py-20">
        <div className="container">
          ...
          <div className="justify-content-center grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {extraDemosData.map((demo, idx) => <DemoCard key={idx} demo={demo} />)}
          </div>
        </div>
      </section>
      <section className="bg-default-50/80 py-20" id="features">
        <div className="container">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {landingFeaturesData.map((feature, idx) => <FeatureCard key={idx} feature={feature} />)}
          </div>
      ...
      <footer style={{
      backgroundImage: `url(${offerBgOtherImg})`
    }} className="relative">
        <div className="pb-10 pt-20">
          <div className="container relative">
            <div className="text-center">
              ...
              <Link className="inline-flex items-center gap-2.5 rounded bg-primary px-4 py-2 text-center text-sm font-semibold text-white shadow-lg  shadow-transparent transition-all duration-500 ease-in-out hover:-translate-y-[2px] focus:outline-none md:me-0" to="">
        ...
        <div className="container relative">
          <p className="py-6 text-center font-medium text-default-900">
            ...
            <Link className="text-primary-700" to="https://coderthemes.com/" target="_blank">
              Coderthemes
            </Link>
          </p>
        </div>
