  ...
  return <div className="w-full lg:ps-64">
      <div className="page-content space-y-6 p-6">
        ...
        <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 2xl:grid-cols-6">
          ...
          return <div key={overview.name + idx} className="flex flex-col justify-between overflow-hidden rounded-lg border border-default-200 p-4 text-center transition-all duration-300 hover:border-primary">
          ...
          <div className="pb-10">
            <div className="space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <Link to="/dishes" className="inline-flex items-center gap-1 text-sm font-medium text-primary hover:text-primary-500">
                  View all <LuChevronRight size={20} />
                </Link>
              </div>
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-4">
                {categoriesData.slice(0, 4).map(category => <Link key={category.id} to="" className="space-y-4 rounded-lg border border-default-200 py-4 text-center transition-colors duration-300 hover:border-primary">
                    <div>
                      <img src={category.image} width={56} height={56} className="mx-auto h-full max-w-full" alt="tea" />
                    </div>
            ...
            <div className="pt-10">
              ...
              <div className="grid grid-cols-2 gap-6 lg:grid-cols-3">
                {dishesData.filter(dish => dish.is_popular).slice(0, 3).map(dish => <BestSellingProductCard key={dish.id} product={dish} />)}
              </div>
            </div>
          </div>

          <div className="pb-10 ">
            <OrderDataTable columns={columns} rows={orderRows.slice(0, 6)} title="Recent Orders" />
          </div>
        </div>
      </div>
    </div>;
