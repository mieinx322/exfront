...
const OrderList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const resolvedStatuses = await Promise.all(Object.keys(orderProgressData).map(async (status, idx) => {
        const orders = await Promise.all(orderProgressData[status].map(async (order, index) => {
          order.order = await getOrderById(order.order_id);
          const dish = await getDishById(order.order?.dish_id ?? 0);
          return <div key={order.order_id + index} className="flex items-center gap-4 rounded-lg bg-primary/10 p-2">
                  <div className="flex h-16 w-16 items-center justify-center">
                    <img src={dish?.images[0] ?? ''} height={64} width={64} alt="food" />
                  </div>
        ...
        return <div className="mb-6" key={status + idx}>
          ...
          <div className="xl:col-span-9">
            <div className="space-y-6">
              <div className="grid gap-6 sm:grid-cols-2 2xl:grid-cols-3">
                ...
                <OrderStatistics title="Your Balance" stats={`${currentCurrency}8,904.80`} icon={LuWallet} variant="bg-yellow-500/20 text-yellow-500" />
                <OrderStatistics title="Satisfaction Rating" stats="98%" icon={FaStar} variant="bg-green-500/20 text-green-500" />
              </div>
              <div className="grid grid-cols-1">
                <OrderDataTable title="Order History" columns={columns} rows={orderRows} />
              </div>
            </div>
          </div>
