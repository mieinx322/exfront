  ...
  return <Controller control={control} defaultValue={''} render={({
    ...
    fieldState
          ...
          <div className={cn('relative', fullWidth && 'w-full')}>
            <DatePicker {...other} {...field} value={field.value ?? ''} className={cn('form-input rounded-lg border border-default-200 px-4 py-2.5 dark:bg-default-50 ', {
        'w-full': fullWidth
      }, {
        'ps-10': startInnerIcon
      }, {
        'border-red-500 focus:border-red-500': !noValidate && fieldState.error?.message
      }, className)} onChange={([date]) => {
        field.onChange(date);
      }} />
