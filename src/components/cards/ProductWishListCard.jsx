...
const ProductWishListCard = ({
  ...
  return <div className="grid grid-cols-3 items-end px-4 py-4 sm:items-center">
      <div className="col-span-2 lg:col-span-1">
        <div className="flex flex-col sm:flex-row sm:items-center">
          <img src={images[0]} width={112} height={112} className="me-2 h-28 w-28 place-self-end sm:place-self-start lg:me-4" alt="sushi" />
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1.5">
                ...
                <div className="hidden gap-1.5 lg:flex">
                  {Array.from(new Array(Math.floor(review.stars))).map((_star, idx) => <FaStar key={idx} size={20} className="fill-yellow-400 text-yellow-400" />)}
                  {!Number.isInteger(review.stars) && <FaStarHalfStroke size={20} className="text-yellow-400" />}
                  {review.stars < 5 && Array.from(new Array(5 - Math.ceil(review.stars))).map((_val, idx) => <FaStar key={idx} size={20} className="text-default-400" />)}
                </div>
              </div>
