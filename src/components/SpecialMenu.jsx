...
const SpecialMenu = () => {
  ...
  return <section className="py-6 lg:py-16">
            ...
            <div className="flex w-full flex-wrap">
              <div className="custom-scroll -mx-4 h-auto w-screen overflow-auto px-2 lg:mx-0 lg:h-[30rem] lg:w-full">
                <nav className="flex gap-2 lg:flex-col" aria-label="Tabs" role="tablist" data-hs-tabs-vertical="true">
                  {categoriesData.map(category => <button type="button" role="tab" key={category.id} className={cn('flex p-1', selectedCategory == category.id && 'active')} id={category.name + '-menu-toggle'} data-hs-tab={'#' + category.name + '-menu'} aria-controls={category.name + '-menu'} onClick={() => setSelectedCategory(category.id)}>
                      <span className="flex w-full items-center justify-start gap-4 rounded-full p-2 pe-6 text-default-900 transition-all hover:text-primary hs-tab-active:bg-primary xl:w-2/3">
                        <div>
                          <span className="inline-flex h-14 w-14 grow items-center justify-center rounded-full hs-tab-active:bg-white">
                            <img src={category.image} height={32} width={32} className="h-8 w-8" alt="category-img" />
                          </span>
                        </div>
            ...
            <div className="relative lg:mt-24">
              ...
              <div className="rounded-lg bg-primary/10 lg:pb-16">
                <div className="p-4 lg:p-6">
                  ...
                  return <div key={category.id} id={category.name + '-menu'} role="tabpanel" aria-labelledby="pizza-menu-item" className={cn(selectedCategory != category.id && 'hidden')}>
              ...
              <div className="hidden lg:flex">
                ...
                <span className="absolute bottom-0 start-1/4 z-10">
                  <img src={onionHomeImg} alt="onion" />
                </span>
                <span className="absolute -bottom-12 -end-0 z-10">
                  <img src={leafHomeImg} alt="leaf" />
                </span>
              </div>
            </div>
          </div>
