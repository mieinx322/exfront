...
const OrderInteraction = lazy(() => import('./OrderInteraction'));
...
const ProductDetailView = ({
  ...
  useEffect(() => {
    (async () => {
      const foundRestaurant = await getRestaurantById(restaurant_id);
      if (foundRestaurant) setRestaurant(foundRestaurant);
    })();
  }, [restaurant_id]);
  ...
  return <div>
      <div className="mb-1 flex flex-wrap items-end justify-between font-medium text-default-800">
        ...
        <h3 className="text-3xl">
          {currentCurrency}
          {discountedPrice}&nbsp;
          {sale && <span className="ms-1 text-base text-gray-400 line-through">
              {currentCurrency}
              {price}
            </span>}
        </h3>
      ...
      {restaurant && <h5 className="mb-2 text-lg font-medium text-default-600">
      ...
      <div className="mb-3 flex items-center gap-3">
        <div className="flex gap-1.5">
          {Array.from(new Array(Math.floor(review.stars))).map((_star, idx) => <FaStar key={idx} size={18} className="text-yellow-400" />)}
          {!Number.isInteger(review.stars) && <FaStarHalfStroke size={18} className="text-yellow-400" />}
          {review.stars < 5 && Array.from(new Array(5 - Math.ceil(review.stars))).map((_val, idx) => <FaStar key={idx} size={18} className="text-default-400" />)}
        </div>
      ...
      <div className="mb-5 flex gap-2">
        ...
        {tags.map((tag, idx) => <div key={tag + idx} className="flex items-center rounded-full border border-default-200 px-3 py-1.5">
      ...
      <div className="mb-8 flex items-center gap-3">
        ...
        {['S', 'M', 'L'].map((size, idx) => <div key={size + idx}>
            <input type="radio" name="size-options" id={size + idx} value={size} className="peer hidden" defaultChecked />
            <label htmlFor={size + idx} className="flex h-9 w-9 cursor-pointer select-none items-center justify-center rounded-full bg-default-200 text-center text-sm peer-checked:bg-primary peer-checked:text-white">
              {size}
            </label>
          </div>)}
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="rounded-lg border border-default-200 p-3">
          <div className="grid grid-cols-4 justify-center">
            {dishNutritionFacts.map((nutrition, idx) => <div key={nutrition.name + idx} className="text-center">
