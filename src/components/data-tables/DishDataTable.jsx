...
const DishDataTable = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      const resolvedCategories = await Promise.all(rows.map(async (row, idx) => {
        const categories = await Promise.all(columns.map(async column => {
          ...
          if (column.key == 'name') {
            return <td key={tableData + idx} className="whitespace-nowrap px-6 py-4 text-sm font-medium text-default-800">
                    <Link to="/admin/dishes/1008" className="flex items-center gap-3">
                      <div className="h-12 w-12 shrink">
                        <img src={row.images[0]} height={48} width={48} alt={tableData} className="h-full max-w-full" />
                      </div>
                      <p className="text-base text-default-500 transition-all hover:text-primary">{tableData}</p>
                    </Link>
                  </td>;
          } else if (column.key == 'status') {
            const colorClassName = tableData == 'published' ? 'bg-green-500/10 text-green-500' : tableData == 'reviewing' ? 'bg-yellow-500/10 text-yellow-500' : 'bg-pink-500/10 text-pink-500';
