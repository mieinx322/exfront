...
const RestaurantMenuDataTable = ({
  ...
  return <div className="overflow-hidden rounded-lg border border-default-200">
                  ...
                  <tr>
                    <th scope="col" className="px-4 py-3 pr-0">
                      <div className="flex h-5 items-center">
                        <input id="hs-table-search-checkbox-all" type="checkbox" className="form-checkbox h-5 w-5 rounded border border-default-300 bg-transparent text-primary focus:border-primary-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50 focus:ring-offset-0" />
                        <label htmlFor="hs-table-search-checkbox-all" className="sr-only">
                          Checkbox
                        </label>
                      </div>
                    </th>
                    {columns.map(column => <th key={column.key} scope="col" className="whitespace-nowrap px-6 py-3 text-start text-base font-medium text-default-500">
                        {column.name}
                      </th>)}
                ...
                <tbody className="divide-y divide-default-200">
                  ...
                  return <tr key={idx}>
                        <td className="py-3 pl-4">
                          <div className="flex h-5 items-center">
                            <input id="hs-table-search-checkbox-1" type="checkbox" className="form-checkbox h-5 w-5 rounded border border-default-300 bg-transparent text-primary focus:border-primary-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50 focus:ring-offset-0" />
                      ...
                      if (column.key == 'name') {
                        return <td className="whitespace-nowrap px-6 py-4" key={tableData + idx}>
                                <div className="flex items-start gap-3">
                                  <img src={row?.images[0] ?? ''} width={48} height={48} className="h-12 w-12 rounded-full" alt={row.name ?? ''} />
                                  <h5 className="text-base font-medium text-default-700">{tableData}</h5>
                                </div>
                              </td>;
                      } else if (column.key == 'review') {
                        return <td key={idx} className="whitespace-nowrap px-6 py-4 text-base text-default-800">
                                <div className="flex items-center justify-start gap-1">
                                  {Array.from(new Array(Math.floor(row.review.stars ?? 0))).map((_star, idx) => <FaStar key={idx} size={18} className="fill-yellow-400 text-yellow-400" />)}
                                  {!Number.isInteger(row.review.stars ?? 0) && <FaStarHalfStroke size={18} className="text-yellow-400" />}
                                  {(row.review.stars ?? 0) < 5 && Array.from(new Array(5 - Math.ceil(row.review.stars ?? 0))).map((_val, idx) => <FaStar key={idx} size={18} className="text-default-400" />)}
                                </div>
                              </td>;
                      } else {
                        return <td key={idx} className="whitespace-nowrap px-6 py-4 text-base text-default-800">
