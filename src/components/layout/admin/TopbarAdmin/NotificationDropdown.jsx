...
const NotificationDropdown = ({
  ...
  return <div className="hs-dropdown relative inline-flex [--placement:bottom-right]">
      <button id="hs-dropdown-with-header" type="button" className="hs-dropdown-toggle inline-flex h-10 w-10 flex-shrink-0 items-center justify-center gap-2 rounded-full bg-default-100 align-middle text-xs font-medium text-default-700 transition-all hover:text-primary">
        <LuBell size={24} />
      ...
      <div className="hs-dropdown-menu duration mt-2 hidden min-w-[20rem] rounded-lg border border-default-200 bg-white opacity-0 shadow-md transition-[opacity,margin] hs-dropdown-open:opacity-100 dark:bg-default-50">
          ...
          if (previousDate !== currentDate) {
            ...
            return <Fragment key={idx}>
                  ...
                  <Link to="" className="mb-4 flex items-center">
                    <img src={notification.avatar} alt="avatar" className="h-10 w-10 rounded-full" />
            ...
            return <Link to="" key={idx} className="mb-4 flex items-center">
                  <div className="flex-shrink-0">
                    <img src={notification.avatar} alt="avatar" height={40} width={40} className="h-10 w-10 rounded-full" />
                  </div>
                  <div className="ms-2 flex-grow truncate">
                    <div className="flex items-center justify-between">
                      <h5 className="text-sm font-medium text-default-800">{notification.name}</h5>
        ...
        <Link to="" className="block px-4 py-2 text-center text-sm font-medium text-primary">
          View All
        </Link>
      </div>
    </div>;
