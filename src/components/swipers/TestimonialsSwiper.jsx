...
const TestimonialsSwiper = ({
  ...
  return <div>
      ...
      <div className="product-img-slider sticky-side-div">
        <Swiper modules={[Thumbs]} spaceBetween={24} thumbs={{
        swiper: thumbsSwiper && !thumbsSwiper.destroyed ? thumbsSwiper : null
      }} navigation={{
        nextEl: '.swiper-button-next',
        prevEl: '.swiper-button-prev'
      }} className="clients-testimonial p-2" wrapperClass="mb-4" loop>
          {reviews.map(review => {
          return <SwiperSlide key={review.id}>
                <div className="relative cursor-pointer">
                  <div className="mb-12 flex items-center gap-3">
                    <img src={review.reviewer_image} height={48} width={48} className="h-12 w-12 rounded-full" alt="avatar" />
                    <div>
                      <h6 className="mb-2 text-base/none font-medium text-default-900">{review.reviewer_name}</h6>
                      <div className="flex gap-1.5">
                        {Array.from(new Array(Math.floor(review.stars))).map((_star, idx) => <FaStar key={idx} size={16} className="fill-yellow-400 text-yellow-400" />)}
        ...
        <Swiper modules={[Navigation, Thumbs]} spaceBetween={10} slidesPerView={4} className="clients-testimonial-pagination relative !mt-6 h-24" wrapperClass="ps-12 !py-6 space-x-2" onSwiper={setThumbsSwiper} watchSlidesProgress loop>
          <SwiperSlide className="!h-12 !w-12 cursor-pointer">
            <img src={avatar1Img} className="h-12 w-12 rounded-full" alt="avatar" />
          </SwiperSlide>
          <SwiperSlide className="!h-12 !w-12 cursor-pointer">
            <img src={avatar2Img} className="h-12 w-12 rounded-full" alt="avatar" />
          </SwiperSlide>
          <SwiperSlide className="!h-12 !w-12 cursor-pointer">
            <img src={avatar3Img} className="h-12 w-12 rounded-full" alt="avatar" />
          </SwiperSlide>
          <SwiperSlide className="!h-12 !w-12 cursor-pointer">
            <img src={avatar4Img} className="h-12 w-12 rounded-full" alt="avatar" />
          </SwiperSlide>
        </Swiper>
      </div>
    </div>;
