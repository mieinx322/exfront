...
const WalletCardSwiper = ({
  ...
  return <div className="relative">
    ...
    }} loop className="card-wallet mb-6 md:!mx-6">
        {cards.map((card, idx) => {
        return <SwiperSlide key={card.id}>
              <div style={{
            backgroundImage: `url(${walletCardBgPaymentImg})`
                ...
                <div className="p-6">
                  <div className="mb-8">
                    <div className="h-11 w-16 overflow-hidden">
                      <img src={card.provider} className="h-full max-w-full" alt="card-provider-company" />
                    </div>
                  </div>
                  <div className="mb-6">
                    <div className="mb-9 flex flex-wrap items-center justify-between gap-y-4">
                      <div className="flex h-8 items-end">
                        <img src={cardChipPaymentImg} width={44.3} height={32} className="h-full max-w-full" alt="chip" />
                      </div>
                      <div className="flex gap-4">
                        {card.cardNo.toString().match(/\d{4}/g)?.map((chunk, idx) => <span key={idx} className="font-sans line-clamp-1 text-xl font-semibold tracking-widest text-white">
                              {chunk}
                            </span>)}
                      </div>
                    </div>
